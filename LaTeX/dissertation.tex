...
\item[Measure] Given a labeled property graph $G = (V, E, L_V, L_E, A_V, A_E)$, a measure is a calculation computed over the graph $G$ using a function $F$, that can return the type of measures defined by  \cite{ghrab2015framework}:
\begin{description}
...
\item[Graph-specific measure] This type of measure is calculated by applying a network analysis algorithm over the graph. In our running example, the list of authors that most contributed with other authors is a graph-specific measure that is computed by applying the degree centrality measure to the \emph{Author} nodes of the graph.
\item[Graph as measure] This kind of measure is given by different aggregation levels of a graph. In our running example, the network of authors and  publications aggregated according to the journal the work was published in is a graph that represents a measure.
\end{description}
\end{description}
 ...
 \caption{Graph Aggregator Process}\label{alg:algorithm1}
  \begin{algorithmic}[1]
    \Function{$generateAggGraph$}{$G, F, D$}
      \State{$aggVertices \gets []$}
      \State{$aggEdges \gets []$}
      \For{$ node \textrm{ in } G.vertices$}
        \If {$ node.label  \textrm{ in } D $}
          \State{$dimValue \gets getDimValue(node, D)$}
           \If {$ dimValue  \textrm{ in } aggVertices $}
             \State{$aggregateVertices(dimValue, node, F)$}
             \State{$aggregateEdges(dimValue, node, F)$}
           \Else
              \State{$aggVertices.add(dimValue, node)$}
              \State{$aggregateEdges(dimValue, node, F)$}
           \EndIf
       \Else
          \State{$aggVertices.add(node)$}
        \EndIf
      \EndFor
      \State{$aggGraph \gets (aggVertices, aggEdges)$}
      \State \Return{$aggGraph$}
    \EndFunction
  \end{algorithmic}
\end{algorithm}
